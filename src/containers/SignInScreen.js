...
const SignInScreen = ({navigation}) => {
    ...
    const login = (_email, _password) => {
        ...
        if (phoneno == '' || pass == '') {
            setisLoading(false);
            return Alert.alert('OOPS!', 'Please enter phone no and password', [
                {text: 'Okay'},
            ]);
        } else {
            // fetch('https://opflex.nurighana.com/api/login', {
            fetch('https://opflextest.nurighana.com/api/login', {
                method: 'POST',
                ...
                .then(resource => {
                    ...
                    if (!resource.data.token) {
                        setisLoading(false);
                        return Alert.alert('OOPS!', 'Invalid Credentials', [
                            {text: 'Okay'},
                        ]);
                    } else {
                        setisLoading(false);
                        ...
                        AsyncStorage.setItem(
                            'user_name',
                            JSON.stringify(resource.data.name),
                        );
                        AsyncStorage.setItem(
                            'user_token',
                            JSON.stringify(resource.data.token),
                        );
                    }
                });
        }
